...
export default function AutomationCanvasPage({ params }: { params: { id: string } }) {
  ...
  const saveAll = async () => {
    ...
    const { error: projErr } = await (supabase
      .from('automation_projects') as any)
    ...
    await (supabase.from('automation_edges') as any).delete().eq('project_id', id).eq('organization_id', orgId)
    await (supabase.from('automation_nodes') as any).delete().eq('project_id', id).eq('organization_id', orgId)
    if (nodes.length) {
      await (supabase.from('automation_nodes') as any).insert(
        nodes.map((n) => ({
          id: n.id,
          project_id: id,
    ...
    if (edges.length) {
      await (supabase.from('automation_edges') as any).insert(
        edges.map((e) => ({
          id: e.id,
          project_id: id,
          organization_id: orgId,
