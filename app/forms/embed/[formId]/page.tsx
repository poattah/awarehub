...
export default function EmbeddableFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect if configured (in parent window)
      if (result.redirect_url) {
        setTimeout(() => {
          if (window.parent !== window) {
            window.parent.location.href = result.redirect_url
          } else {
  ...
  if (error && !form) {
    return (
      <div className="min-h-[400px] flex items-center justify-center bg-transparent px-4">
        <Card className="w-full p-6 text-center space-y-3">
          <div className="mx-auto h-12 w-12 rounded-full bg-red-100 flex items-center justify-center">
            <X className="h-6 w-6 text-red-600" />
          </div>
          <div>
            <h2 className="text-lg font-bold">Form Not Available</h2>
  ...
  if (success) {
    return (
      <div className={`min-h-[400px] flex items-center justify-center ${bgClass} p-4`}>
        <Card className="w-full p-6 text-center space-y-3">
          <div
  ...
  return (
    <div className={`min-h-[400px] ${bgClass} p-4`}>
